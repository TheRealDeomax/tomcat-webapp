...
For secure connections, use the SSL-enabled configuration:

```powershell
# Generate SSL certificate and start HTTPS setup
.\setup-ssl.ps1
```

**HTTPS Access Points:**
- **Secure App**: https://localhost (HTTPS with SSL)
- **HTTP Redirect**: http://localhost (redirects to HTTPS)
- **Manager App**: https://localhost/manager/html
- **File Upload**: https://localhost/file-upload/

**SSL Certificate Options:**
1. **Development**: Self-signed certificate (generated by setup-ssl.ps1)
2. **Production**: Let's Encrypt free certificates
3. **Enterprise**: Commercial certificates from trusted CAs
